// Shared Playwright utilities for optimized web scraping (Node.js version)
 ...
 * Reuses browser for better performance
 */
async function getBrowser(): Promise<Browser> {
  ...
  try {
    ...
    throw new Error(`Playwright browser launch failed: ${errorMsg}`);
  }
}
...
async function createOptimizedPage(
  ...
  if (interceptJsonUrls && interceptJsonUrls.length > 0) {
    context.on('response', async (response) => {
      ...
      const matchesPattern = interceptJsonUrls.some((pattern) => url.includes(pattern));
      
      // Also intercept if URL looks like it could be a data API
      const isLikelyDataApi = url.includes('api') || url.includes('quotes') || url.includes('proxies') || url.includes('core-api') || url.includes('quotes/get');
      
      // Accept JSON or text responses that might be JSON
      const couldBeJson = contentType.includes('json') || contentType.includes('text') || contentType === '' || url.includes('quotes/get');
      
      if ((matchesPattern || isLikelyDataApi) && couldBeJson) {
        try {
          ...
          if (text.startsWith('{') || text.startsWith('[')) {
            ...
            console.log(`Intercepted JSON from: ${url.substring(0, 150)}...`);
            // Log if it's a quotes/get API
            if (url.includes('quotes/get')) {
              console.log(`  -> This is a quotes/get API response`);
            }
          }
        } catch (e) {
          // Ignore parse errors
  ...
  page.on('request', (request) => {
    if (request.resourceType() === 'xhr' || request.resourceType() === 'fetch') {
      console.log(`XHR/Fetch request: ${request.url().substring(0, 150)}`);
    }
  });
